        ...
        <div id="hero-slider" class="hero-slider">
            @if (iterator_count($trip->trip_galleries))
                @foreach ($trip->trip_galleries as $gallery)
                    <div class="slide">
                        <img src="{{ $gallery->imageUrl }}" class="object-cover h-96 md:h-[36rem] lg:h-[48rem]" alt="">
                    </div>
                @endforeach
            @endif
        </div>
                    ...
                    <ul class="flex flex-wrap gap-1 py-1 nav">
                        ...
                        <li >
                            <a href="#gallery" class="flex items-center p-2 hover:bg-white hover:text-primary">
                                <svg fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" class="w-6 h-6 md:mr-2" xmlns="http://www.w3.org/2000/svg"
                                    aria-hidden="true">
            ...
            <div class="grid gap-2 lg:grid-cols-3 xl:grid-cols-4 lg:gap-10">
                        ...
                        <div class="p-3 bg-white">
                            @if ($trip->trip_include_exclude)
                                <div class="grid gap-10 lg:grid-cols-2">
                                    <div>
                                        <h2 class="mb-4 text-xl text-gray-600 lg:text-2xl ">What's Includes</h2>
                                        <ul class="includes">
                                    ...
                                    <div>
                                        <h2 class="mb-4 text-xl text-gray-600 lg:text-2xl ">What's Exclude</h2>
                                        <ul class="excludes">
                    ...
                    @if (!$trip->trip_departures->isEmpty())
                        <div id="date-price" class="px-4 py-10 mb-4 bg-white rounded-lg lg:px-10 tds">
                            ...
                            <div id="all-dates-block" class="grid grid-cols-4 gap-2 mb-4 md:grid-cols-6 lg:grid-cols-9">
                                <button id="all-departure-filter" class="p-2 px-4 py-2 font-semibold text-center text-white border rounded departure-date-active border-primary bg-primary"> All <br> Dep</button>
                                @foreach($monthsArray as $month)
                                    <button data-date="{{ $month }}" class="p-2 px-4 py-2 font-semibold text-center border border-gray-200 rounded select-date-departure hover:border-primary hover:text-primary">{{ Str::replaceFirst('-', '<br>', date('M Y', $month)) }}</button>
                                @endforeach
                            </div>
                                    ...
                                    @foreach ($trip_departures as $departure)
                                        <div class="relative grid grid-cols-2 gap-4 p-4 border border-gray-200 rounded lg:grid-cols-5 lg:place-items-center hover:border-primary">
                                            ...
                                            <div class="flex items-center">
                                                <a href="{{ route('front.trips.departure-booking', ['slug' => $trip->slug, 'id' => $departure->id]) }}" class="px-3 py-2 text-sm border rounded border-primary text-primary hover:bg-primary hover:text-white">Book Now</a>
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                ...
                <aside>
                    ...
                    @if (iterator_count($trip->addon_trips))
                        <div class="mb-8">
                            <h2 class="mb-2 text-2xl text-gray-100 font-display">Additional Tours</h2>
