...
@push('styles')
<link href="{{ asset('assets/vendors/cropperjs/dist/cropper.min.css') }}" rel="stylesheet">
@endpush
@section('content')
...
$(function() {
  ...
  function handleSliderForm(form) {
    var form = $(form);
    ...
    $.ajax({
        url: "{{ route('admin.trips.slider.update') }}",
        type: 'POST',
        data: formData,
        dataType: 'json',
        processData: false,
        ...
        success: function(res) {
            if (res.status === 1) {
                location.href = '{{ route("admin.trips.edit", $slider->trip_id) }}';
            }
        }
    });
  }
